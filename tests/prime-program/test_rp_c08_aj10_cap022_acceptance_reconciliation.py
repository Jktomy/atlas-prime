...
class Aj10Cap022AcceptanceReconciliationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.register = json.loads((ROOT / "governance/capability-parity-register.json").read_text(encoding="utf-8"))
    ...
    def load_bindings(self, scope: str) -> list[dict]:
        return [json.loads((ROOT / binding["path"]).read_text(encoding="utf-8")) for binding in self.proof["accepted_record_bindings"][scope]]
    ...
    def test_seven_immutable_records_and_living_emberline_identity_match(self) -> None:
        ...
        for scope in ("admitted_quest", "nonquest"):
            for binding in self.proof["accepted_record_bindings"][scope]:
                ...
                if record["schema_id"] == "atlas.lifecycle.quest-emberline":
                    ...
                    self.assertEqual(self.proof["fresh_context_readback"]["quest_emberline_id"], binding["record_id"])
        ...
        self.assertEqual(historical_qem["sha256"], "9174ed49302f666b40a60669196ebe2f81a66a942e9cb5b5a141e23ed9a424ed")
    ...
    def test_admitted_quest_resolves_historical_pair_and_current_quest_state(self) -> None:
        ...
        self.assertEqual({key: len(value) for key, value in by_schema.items()}, {"atlas.lifecycle.feather": 1, "atlas.lifecycle.sunset": 1, "atlas.lifecycle.sunrise": 1, "atlas.lifecycle.quest-checkpoint": 1, "atlas.lifecycle.quest-emberline": 1})
        ...
        self.assertEqual(historical["next_gate"], "MERGE_THEN_FRESH_CONTEXT_AJ10_CAP022_READBACK")
        self.assertEqual(emberline["record_id"], historical["quest_emberline_id"])
        qem_binding = next(binding for binding in self.proof["accepted_record_bindings"]["admitted_quest"] if binding["record_id"] == emberline["record_id"])
        self.assertEqual(qem_binding["sha256"], "9174ed49302f666b40a60669196ebe2f81a66a942e9cb5b5a141e23ed9a424ed")
        context = compact_context(self.snapshot, quest_id="repairing-prime", projection_warning="Non-authoritative lifecycle website projection is missing.")
        self.assertEqual(context["latest_valid_feather"], emberline["latest_feather_id"])
        self.assertEqual(context["next_gate"], "CLOSED")
    ...
    def test_aj10_record_transitions_only_its_authorized_dispositions(self) -> None:
        ...
        self.assertEqual(self.proof["transitions"]["CAP-022"], {"from": "STILL_MISSING/MISSING", "to": "RESTORED/ACTIVE"})
    ...
    def test_continuity_preserves_prior_gates_and_records_final_closeout(self) -> None:
        ...
        ordered = (gate_three_event, gate_four_event, m07_event, aj11_event, aj12_event, cap027_event, strikeforce_event, recovery_event, completion_event)
        ...
        self.assertNotIn("CONT-REPAIRING-PRIME-R01", {entry["continuity_id"] for entry in self.continuity["entries"]})
