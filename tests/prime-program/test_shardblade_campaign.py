...
class CampaignShardbladeTests(unittest.TestCase):
    ...
    def warrant(self) -> dict:
        return {
            ...
            "gemstone_id": "GEM-LESSON-HARVEST-R02", "authorizer": "Jayson", "repository": "Jktomy/atlas-prime",
            "issued_at": "2026-07-18T12:00:00Z", "expires_at": "2026-07-21T12:00:00Z", "status": "ACTIVE",
            "stages": [
                {"stage_id": 0, "route": "AEGIS_BREAK_BOOTSTRAP", "base_sha": "a" * 40, "allowed_paths": ["governance/atlas-aegis.md"], "actions": ["READY", "MERGE"]},
                {"stage_id": 1, "route": "CAMPAIGN_SHARDBLADE", "base_sha": "b" * 40, "allowed_paths": ["governance/lesson-harvest-protocol.md"], "actions": ["READY", "MERGE"]},
            ],
    ...
    def request(self, warrant: dict) -> dict:
        return {
            ...
            "base_sha": "a" * 40, "pull_request": 240, "pr_state": "DRAFT", "source_branch": "campaign-r02-t0", "head_sha": "c" * 40, "tree_sha": "d" * 40,
    ...
    def test_exact_ready_merge_chain(self) -> None:
        ...
        validate_campaign_warrant(warrant, now=self.now)
        validate_stage_request(request, warrant, now=self.now)
        validate_stage_receipt(receipt, request, warrant)
        validate_stage_request(self.merge(request, receipt), warrant, prior_ready_receipt=receipt, now=self.now)
    ...
    def test_scope_expiry_reviews_checks_and_terminal_state_fail_closed(self) -> None:
        ...
        for mutate, code in (
            ...
            with self.assertRaisesRegex(WarrantValidationError, code): validate_campaign_warrant(candidate, now=self.now)
        request = self.request(warrant); request["changed_paths"] = ["governance/change-routes.md"]
        with self.assertRaisesRegex(WarrantValidationError, "CAMPAIGN_SCOPE_WIDENED"): validate_stage_request(request, warrant, now=self.now)
        request = self.request(warrant); request["checks"][0]["head_sha"] = "f" * 40
        with self.assertRaisesRegex(WarrantValidationError, "CAMPAIGN_CHECK_HEAD_MISMATCH"): validate_stage_request(request, warrant, now=self.now)
        request = self.request(warrant); request["copilot_dispositions"] = [{"id": "c1", "classification": "ACTIONABLE", "reason": "repair"}]
        with self.assertRaisesRegex(WarrantValidationError, "CAMPAIGN_REVIEW_UNRESOLVED"): validate_stage_request(request, warrant, now=self.now)
    ...
    def test_merge_requires_successful_ready_fresh_readback_and_exact_candidate(self) -> None:
        ...
        with self.assertRaisesRegex(WarrantValidationError, "CAMPAIGN_READY_RECEIPT_REQUIRED"): validate_stage_request(merge, warrant, now=self.now)
        drifted = copy.deepcopy(merge); drifted["head_sha"] = "e" * 40
        for check in drifted["checks"]: check["head_sha"] = drifted["head_sha"]
        with self.assertRaisesRegex(WarrantValidationError, "CAMPAIGN_CANDIDATE_DRIFT"): validate_stage_request(drifted, warrant, prior_ready_receipt=receipt, now=self.now)
        stale = copy.deepcopy(merge); stale["readback_at"] = receipt["executed_at"]
        with self.assertRaisesRegex(WarrantValidationError, "CAMPAIGN_FRESH_READBACK_REQUIRED"): validate_stage_request(stale, warrant, prior_ready_receipt=receipt, now=self.now)
