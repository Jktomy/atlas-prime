...
class RpC08CapabilityReconciliationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.proof = json.loads((ROOT / "proof/repairing-prime/rp-c08-cap010-reconciliation-r01.json").read_text(encoding="utf-8"))
        self.cap011_proof = json.loads((ROOT / "proof/repairing-prime/rp-c08-cap011-reconciliation-r01.json").read_text(encoding="utf-8"))
    ...
    def test_cap011_is_restored_only_by_live_multifile_evidence(self) -> None:
        ...
        evidence = self.cap011_proof["hosted_multifile_evidence"]
    ...
    def test_cap010_is_restored_only_by_guided_construction_and_live_evidence(self) -> None:
        records = {record["id"]: record for record in self.register["capabilities"]}
        self.assertEqual(records["CAP-010"]["capability_disposition"], "RESTORED")
        self.assertEqual(records["CAP-010"]["activation_state"], "ACTIVE")
        construction = self.proof["guided_publisher_construction"]
        evidence = self.proof["guided_live_evidence"]
    ...
    def test_fresh_work_and_final_acceptance_boundaries_remain_missing(self) -> None:
        records = {record["id"]: record for record in self.register["capabilities"]}
        self.assertEqual(set(self.proof["still_missing"]), {"CAP-015", "CAP-027"})
