...
SEMANTIC_KEYS = (
    "expected_main_sha", "project_id", "operation_id", "quest_scope",
    "campaign", "mission", "gate", "context_summary", "completion_assessment",
    "decisions", "open_items", "current_position", "unresolved_blockers",
    "next_safe_action", "next_approval_gate", "next_gate",
    "durable_source_references", "evidence_pointers", "protected_data",
    "lesson_harvest",
)
SCHEMA_DIRECTORY = {
...
def _validate_draft_request(root: Path, request_path: Path) -> dict[str, Any]:
    ...
    if request.get("schema_id") != "atlas.lifecycle.sunset-request" or request.get("schema_version") != "2.0.0":
    ...
    if scope.get("scope_type") == "ADMITTED_QUEST":
        ...
        if not any(
            item.get("authority") == "CANONICAL_SOURCE"
    ...
    *,
    ...
    preview = {
        ...
        "preview_id": _stable_id(
            "SPV",
            {"semantic_digest": semantic_digest, "expected_main_sha": request["expected_main_sha"]},
        ),
...
def verify_sunset_preview(
    ...
    *,
    ...
    if [path.name for path in members] != [PREVIEW_ARTIFACT, PREVIEW_RECEIPT]:
    ...
    expected_id = _stable_id(
        "SPV",
        {"semantic_digest": semantic_digest, "expected_main_sha": preview["expected_main_sha"]},
    )
    expected_receipt = {
        "schema_id": "atlas.lifecycle.sunset-preview-receipt",
        "schema_version": "1.0.0",
...
def generate_sunset_approval(
    ...
    *,
    ...
    permanence_mode = "SHARDBLADE" if approval_mode == "GODDESS_MODE_SHARDBLADE" else "MANUAL_MERGE"
    approval_seed = {
        "preview_id": preview["preview_id"],
        "preview_digest": verified["preview_digest"],
    ...
    result = verify_sunset_approval(root, preview_dir, destination, require_current_head=True)
    return {
        "authority": "ROUTE_NEUTRAL_APPROVED_CARRIER",
...
def verify_sunset_approval(
    ...
    *,
    ...
    root = repo_root.resolve()
    preview_result = verify_sunset_preview(root, preview_dir, require_current_head=require_current_head)
    ...
    expected_names = sorted([APPROVAL_ARTIFACT, CARRIER_ARTIFACT, BOUND_REQUEST, APPROVAL_RECEIPT])
    if names != expected_names:
        _fail("SUNSET_APPROVAL_MEMBERS", "Sunset approval has unexpected members")
    approval = _read_canonical(approval_dir / APPROVAL_ARTIFACT, "SUNSET_APPROVAL_CANONICAL")
    carrier = _read_canonical(approval_dir / CARRIER_ARTIFACT, "SUNSET_APPROVAL_CANONICAL")
    request = _read_canonical(approval_dir / BOUND_REQUEST, "SUNSET_APPROVAL_CANONICAL")
    receipt = _read_canonical(approval_dir / APPROVAL_RECEIPT, "SUNSET_APPROVAL_CANONICAL")
    _dynamic_validate(root, approval, "sunset-approval-v1.schema.json")
    _dynamic_validate(root, carrier, "sunset-carrier-v1.schema.json")
    _dynamic_validate(root, request, "sunset-request-v3.schema.json")
    approval_digest = _digest(canonical_bytes(approval))
    ...
    if approval["approval_id"] != expected_approval_id or carrier["carrier_id"] != expected_carrier_id:
...
def _validate_records(root: Path, records: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, int]]:
    validator = SchemaValidator(root / "lifecycle" / "schemas")
    bindings = []
    ...
    for record in records:
        ...
        if record.get("authority") != "CANONICAL_RECORD" or stable_record_id(record) != record.get("record_id"):
    ...
    _validate_lesson_harvest_bindings([*snapshot.canonical_records, *records], record_class="candidate")
    counts = {
        ...
        "quest_emberlines": sum(r["schema_id"] == "atlas.lifecycle.quest-emberline" for r in records),
        "quest_checkpoints": sum(r["schema_id"] == "atlas.lifecycle.quest-checkpoint" for r in records),
...
def generate_approved_sunset_candidate(
    ...
    root = repo_root.resolve()
    approved = verify_sunset_approval(root, preview_dir, approval_dir, require_current_head=True)
    request = _read_canonical(request_path, "SUNSET_REQUEST_CANONICAL")
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        legacy_request_path.write_bytes(canonical_bytes(legacy_request))
        ...
        legacy_bundle = _read_canonical(legacy_dir / ARTIFACT_BUNDLE, "SUNSET_CANDIDATE_CANONICAL")
    ...
    expected = {**approved["preview"]["expected_records"], "quest_identity_fabricated": False}
    if assertions != expected:
        _fail("SUNSET_RECORD_PLAN", "candidate records differ from approved Preview")
    entries = [{"path": _record_path(record), "record": record} for record in sorted(records, key=_record_path)]
    bundle = {
    ...
    candidate_set_digest = _digest(canonical_bytes({
...
def verify_approved_sunset_candidate(repo_root: Path, candidate_dir: Path) -> dict[str, Any]:
    root = repo_root.resolve()
    ...
    bundle = _read_canonical(candidate_dir / ARTIFACT_BUNDLE, "SUNSET_CANDIDATE_CANONICAL")
    receipt = _read_canonical(candidate_dir / ARTIFACT_RECEIPT, "SUNSET_CANDIDATE_CANONICAL")
    if (
        bundle.get("schema_id") != "atlas.lifecycle.approved-sunset-candidate-bundle"
        or receipt.get("schema_id") != "atlas.lifecycle.approved-sunset-candidate-receipt"
    ...
    if [entry["path"] for entry in bundle["records"]] != [item["path"] for item in bindings]:
