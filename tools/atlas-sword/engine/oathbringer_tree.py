...
def build_tree_elements(mission: dict[str, Any], package_root: Path, reference_tree: dict[str, dict[str, Any]], client: Any) -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    source_description = 'expected base'
    if mission.get('lane') == 'REPAIR':
        _, source_tree = client.get_tree_for_commit(str(mission['expected_head']))
        source_description = 'current pull-request head'
    ...
    for item in mission['declared_paths']:
        ...
        target = item['path']
        source_target = source_tree.get(target)
        if operation == 'ADD':
            _require(source_target is None, f'ADD target already exists in {source_description}: {target}')
            ...
            _require(source_target is not None, f'REPLACE target missing from {source_description}: {target}')
            _require(source_target['sha'] == item['source_blob'], f'source blob drift for {target}')
            blob = client.create_blob(_payload_bytes(package_root, item))
            elements.append({'path': target, 'mode': source_target.get('mode', '100644'), 'type': 'blob', 'sha': blob})
            payload_blobs[target] = blob
        elif operation == 'DELETE':
            _require(source_target is not None, f'DELETE target missing from {source_description}: {target}')
            _require(source_target['sha'] == item['source_blob'], f'source blob drift for {target}')
            elements.append({'path': target, 'mode': '100644', 'type': 'blob', 'sha': None})
        else:
            source = item['source_path']
            source_entry = source_tree.get(source)
            _require(source_entry is not None, f'{operation} source missing from {source_description}: {source}')
            _require(source_entry['sha'] == item['source_blob'], f'source blob drift for {source}')
            _require(source_target is None, f'{operation} destination exists in {source_description}: {target}')
            if item.get('payload_path') is not None:
                blob = client.create_blob(_payload_bytes(package_root, item))
            else:
                blob = str(source_entry['sha'])
            elements.append({'path': target, 'mode': item.get('mode', source_entry.get('mode', '100644')), 'type': 'blob', 'sha': blob})
