...
The campaign-specific hosted comment ingress is retired after completion of
Mission #257. `.github/workflows/sunset-router-preview-intake.yml` is
intentionally absent so unrelated Issue comments do not create no-job workflow
runs or failed-run notifications.

The closed `atlas.sunset-router.preview-intake` schema and
`tools.sunset_router.issue_preview_ingress` adapter remain frozen historical
evidence and local regression fixtures. They do not accept live hosted intake,
grant authority, or establish an Athena-callable route. A runner guard must not
be added merely to make unrelated comment events appear successful because that
would consume hosted minutes without performing Sunset work.

Use the deterministic CLI above for current Sunset Router operation. Restoring
hosted ingress requires a new separately authorized transport contract with a
trigger surface that does not run or create no-job workflow records for
unrelated repository comments.
