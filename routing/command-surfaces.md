...
| Guided Spear Preview and Execute | `python -B -m tools.athena_routes.guided_publisher preview|execute`; closed receipts are defined by `schemas/athena-guided-intake-preview-v1.schema.json` and `schemas/athena-guided-intake-execute-receipt-v1.schema.json`; component presence does not prove CAP-010 |
