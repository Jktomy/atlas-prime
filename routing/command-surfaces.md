...
| Mission #257 owner-only Sunset Preview ingress | Post exactly one canonical `atlas-sunset-router-preview-intake-v1` block as a new owner comment on Mission #257; `.github/workflows/sunset-router-preview-intake.yml` validates the exact repository, Issue, owner, current-main base, Athena Spear route, replay identity, size, schema, and public-clean boundary, invokes only `tools.sunset_router preview` in temporary storage, and returns the exact Preview and receipt digests for a separate Jayson approval |
