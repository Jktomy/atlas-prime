...
| Active Quests and admitted Quest parents | `continuity/mission-board-quest-registry-r01.json`, live parent Issues `#307`–`#309`, then `quests/`; `quest-board/quest-board-v1.json` is frozen predecessor evidence only |
...
| Admit a later Quest | Create and bind one unique `mission/quest` parent Issue, add one schema-valid Quest source and one schema-valid registry row, increment the registry revision exactly once, validate against the frozen predecessor, then use the reviewed draft-PR route; the frozen Quest Board rejects admission |
