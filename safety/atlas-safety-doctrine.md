...
Stop before mutation when the base moved, a declared path or source blob differs, private data is exposed, the intended action exceeds the mission, rollback is unproven, an identity is ambiguous, or evidence cannot be read back independently.

Fail closed on malformed input, duplicate paths, traversal, symlinks, case-fold collisions, undeclared bytes, duplicate branches or PRs, stale bases, replay, partial state, and unsupported command routes.

## Completion rule

Completion requires the merged main head, its tree and path set, all relevant checks, review state, recovery proof, rollback proof, and clean-clone restoration to agree. Planning, draft source, a package, a branch, or a green local test alone is not completion.
