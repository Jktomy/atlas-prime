...
**Owning Operation:** Operation Nexus  
**Supporting Projects:** Codex, Odyssey, Phoenix, Helios  
...
- Prime Ascendant owns future application semantics, Living Memory, Nexus Core, Dawnshard, the private control surface, local intelligence coordination, workflow refraction, and future Gitea cutover semantics.
- Prime Git remains durable doctrine and accepted source; PostgreSQL Living Memory is future operational state; Nexus Core is the future governed writer; Dawnshard is a restart-safe projection; PostgreSQL full-text search plus `pgvector` is the initial retrieval direction; Qdrant is deferred until demonstrated need.
...
|---|---|---|---|
| PA-C01 — Write the Covenant | Artemis / Nexus with Codex | Constitutional boundaries, source/memory relationships, protected data, rationale, unresolved decisions, and acceptance criteria. | `ACTIVE — ARCHITECTURE REFINEMENT` |
| PA-C02 — Raise the Living Archive | Artemis / Nexus with Phoenix | PostgreSQL schemas, events, audit, retrieval, export, backup, restore, WAL, PITR, and reconciliation. | Depends on PA-C01 and substrate gates. |
| PA-C03 — Forge Nexus Core | Artemis / Nexus | Governed application writer, authorization, receipts, idempotency, integrations, failure, and rollback. | Depends on PA-C01 and PA-C02 contracts. |
| PA-C04 — Shape the Dawnshard | Artemis / Nexus | Watermarked, stale-aware, fail-closed, restart-safe reconciliation projection. | Depends on a stable event contract. |
| PA-C05 — Open the Private Gate | Artemis / Nexus with Codex | Private website/API, authentication, approvals, health, receipts, read views, and bounded actions. | Depends on Nexus contracts; no public endpoint. |
| PA-C06 — Crown Gitea | Codex / Nexus with Phoenix | Shadow parity, backup, mirror, recovery, rollback, access, and future cutover evidence. | Cutover remains a separate Jayson gate. |
| PA-C07 — Awaken Artemis | Artemis / AI Governance | Bounded retrieval, embeddings, reranking, OCR, assistance, and resource measurement. | Safe interfaces required. |
| PA-C08 — Bind the Realms | Artemis with Odyssey, Phoenix, Codex, Helios | Integrate components, prove boundaries and degraded modes, and keep monitoring nonauthoritative. | Component contracts and substrate gates. |
...
PA-C01 may proceed as source architecture now. PA-C02 and PA-C03 require appropriate Prometheus's Fire gates before deployment; PA-C04 requires a stable event contract; PA-C05 requires Nexus authorization and read/write contracts; PA-C06 starts in shadow and cannot authorize cutover; PA-C07 may follow safe interfaces; PA-C08 requires proven boundaries; PA-C09 requires old and new evidence; and PA-C10 requires the complete evidence chain. Notum's Watch may remain parallel-safe and nonauthoritative.
