...
- **Emberdark** — dedicated QEMU VM substrate for governed transit, mission state, and the initial Coppermind PostgreSQL services; no broad infrastructure or source authority.
...
Loss or shutdown of Apollo, Hermes, Iris, or Zeus must not stop persistent Forge automation, qBittorrent, Audiobookshelf, current Plex, access to finalized Anvil media, Prime source recovery, or administration through another approved route.
...
- Coppermind Phase 1 retrieval uses PostgreSQL relational state, PostgreSQL full-text
...
- Failure of observability, Emberdark, Kandra, or generated projections must not block Prime source recovery.
