...
Activation requires the exact authorizer and declared evidence. Suspension
blocks new action without erasing evidence. Revocation is immediate and
terminal. Expiry is terminal. Replacement creates a new warrant with its own
scope and evidence; authority is never inherited silently.
...
approval record, evidence, result, stop, rollback, and forbidden confirmation.
...
The machine contract is `schemas/agentic-capability-warrant-v1.schema.json`.
Fixtures and whole-program tests prove structural closure and lifecycle
semantics. This contract grants no capability or journey promotion by itself.
