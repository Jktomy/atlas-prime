---
title: "Atlas Quest Portfolio Contract"
atlas_id: "prime.governance.quest-portfolio-r01"
...
private_boundary: "Public-clean ownership, dependency, parent-Mission identity, and gate state only. Protected evidence and runtime facts remain in their approved systems."
---

# Atlas Quest portfolio contract

## Portfolio rule

Atlas is the umbrella ecosystem, not a Quest. A durable responsibility has one
Quest owner, one owning Operation, and explicit supporting owners. Transfer does
...
| canonical source governance | Codex / Operation Source Governance | Prime remains canonical; Mission Issues, registry snapshots, generated projections, clients, and runtime stores do not govern authored doctrine |
...
- Prime Reborn and Repairing Prime remain completed immutable history. Their
  proof, continuity, identity, recovery, and lifecycle records are never
  rewritten by portfolio or registry changes.

## Registry boundary

The frozen `quest-board/quest-board-v1.json` preserves the exact seven-Quest
pre-cutover state and is not deleted. It is no longer a work queue, admission
surface, or operational registry. Future Quests receive one unique parent Issue
and one reviewed registry row. Child Missions name their parent Quest identity
but cannot replace or duplicate the parent.

No split-brain interval is permitted:

- before the Mission #278 merge, the frozen candidate metadata and parent Issues
  are non-authoritative preparation;
- at the exact merge, the portable Mission Board registry becomes authoritative
  and the Quest Board becomes frozen predecessor evidence in the same tree; and
- after merge, rollback requires one reviewed revert or repair-forward PR.
...
Issues as not planned if the cutover is cancelled. After merge, use a reviewed
revert or repair-forward PR restoring the prior registry authority; never
rewrite Issue history, Git history, completed Quest history, or protected
evidence.
